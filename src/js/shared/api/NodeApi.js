...
function shouldRetryRequest(err) {
    ...
    if (
        !err.response &&
        config.currentRetryAttempt >= config.noResponseRetries
    ) {
        return false;
    }
...
export const nodeApi = (baseUrl, useCustomRequestConfig) => {
    ...
    retryableAxios.defaults.raxConfig = buildRetryableAxiosConfig(
        retryableAxios
    );
    rax.attach(retryableAxios);
    const retryableGet = (url, config) =>
        retryableAxios.get(trimmedUrl + url, config);
    ...
    return {
        ...
        addresses: {
            details: (address) =>
                retryableGet(`/addresses/balance/details/${address}`),
            aliases: (address) => retryableGet(`/alias/by-address/${address}`),
            validate: (address) =>
                retryableGet(`/addresses/validate/${address}`),
            data: (address) => retryableGet(`/addresses/data/${address}`),
            scriptInfo: (address) =>
                retryableGet(`/addresses/scriptInfo/${address}`),
            scriptMeta: (address) =>
                retryableGet(`/addresses/scriptInfo/${address}/meta`),
        },
        blocks: {
            height: () => get("/blocks/height"),
            heightBySignature: (signature) =>
                get(`/blocks/height/${signature}`),
            delay: (fromSignature, count) =>
                get(`/blocks/delay/${fromSignature}/${count}`),
            at: (height) =>
                retryableGet(`/blocks/at/${height}`, {
                    transformResponse: axios.defaults.transformResponse.concat(
                        transformTimestampToDateTime
                    ),
                }),
            headers: {
                last: () =>
                    retryableGet("/blocks/headers/last", {
                        transformResponse: axios.defaults.transformResponse.concat(
                            transformTimestampToDateTime
                        ),
                    }),
                at: (height) =>
                    retryableGet(`/blocks/headers/at/${height}`, {
                        transformResponse: axios.defaults.transformResponse.concat(
                            transformTimestampToDateTime
                        ),
                    }),
                sequence: (from, to) =>
                    retryableGet(`/blocks/headers/seq/${from}/${to}`, {
                        transformResponse: axios.defaults.transformResponse.concat(
                            transformTimestampToDateTime
                        ),
                    }),
        ...
        transactions: {
            ...
            status: async (idsArray) => {
                ...
                const res = await Promise.all(
                    subarray.map(
                        async (id) =>
                            (
                                await axios.get("/transactions/status", {
                                    baseURL: baseUrl,
            ...
            address: (address, limit, after) => {
                ...
                return retryableGet(
                    `/transactions/address/${address}/limit/${top}`,
                    config
                );
            },
            stateChanges: (id) =>
                retryableGet(`/debug/stateChanges/info/${id}`),
        },
        aliases: {
        ...
        assets: {
            ...
            details: (assetId, full) =>
                retryableGet(`/assets/details/${assetId}`, {
                    params: {
                        full: !!full,
                    },
                }),
