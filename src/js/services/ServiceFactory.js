...
class NetworkDependentServices {
    constructor(globalServices, networkId) {
        ...
        this._currencyService = new CurrencyService(
            globalServices.configurationService(),
            new SafeCurrencyCache(
                database
                // globalServices.errorReportingService()
            ),
            networkId
        );
        this._spamDetectionService = new SpamDetectionService(
            globalServices.storageService(),
            globalServices.configurationService(),
            networkId
        );
        this._stateChangeService = new StateChangeService(
            globalServices.configurationService(),
            networkId
        );
        ...
        this._transactionTransformerService = new TransactionTransformerService(
            this._currencyService,
            this._spamDetectionService,
            this._stateChangeService,
            this._assetService
        );
        ...
        this._aliasService = new AliasService(
            globalServices.configurationService(),
            networkId
        );
    }
...
class ServiceFactory {
    ...
    forNetwork = (networkId) => {
        ...
        if (!services) {
            services = new NetworkDependentServices(
