...
export class SingleBlockPage extends React.Component {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (height !== prevHeight || networkId !== prevNetworkId) {
            ...
            this.fetchData(height).catch((error) => {
    ...
    render() {
        ...
        return (
            <Loader
                ...
                errorTitle="Failed to load block"
            >
                <div className="content card">
                    <GoBack />
                    <Headline
                        title="Block"
                        subtitle={this.state.currentHeight.toString()}
                        copyVisible={false}
                    />
                    ...
                    {Object.keys(this.state.groupedTransactions).map((type) => {
                        ...
                        return (
                            <TransactionList
                                ...
                                header={header}
                                transactions={
                                    this.state.groupedTransactions[type]
                                }
                            />
                        );
                    })}
    ...
    stateToDictionaryItems() {
            ...
            default: [
                {
                    ...
                    value: (
                        <Enumerator
                            ...
                            onNext={() =>
                                this.showBlock(this.state.currentHeight + 1)
                            }
                            onPrev={() =>
                                this.showBlock(this.state.currentHeight - 1)
                            }
                ...
                },
                {
                    label: "Generator",
                    value: (
                        <EndpointRef endpoint={this.state.block.generator} />
                    ),
                },
                {
                    label: "Signature",
                ...
                },
                {
                    ...
                    value: (
                        <React.Fragment>
                            {this.state.block.blocksize} bytes
                        </React.Fragment>
                    ),
                },
                {
        ...
        if (this.state.block.version === 5) {
            ...
            items.default.push({
                ...
                value: this.state.block.transactionsRoot,
            });
        }
