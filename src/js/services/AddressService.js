...
export class AddressService extends ApiClientService {
    ...
    loadBalance = (address) => {
        return this.getApi()
            ...
            .then((balanceResponse) => {
                ...
                return {
                    regular: Money.fromCoins(
                        data.regular,
                        Currency.WAVES
                    ).toString(),
                    generating: Money.fromCoins(
                        data.generating,
                        Currency.WAVES
                    ).toString(),
                    available: Money.fromCoins(
                        data.available,
                        Currency.WAVES
                    ).toString(),
                    effective: Money.fromCoins(
                        data.effective,
                        Currency.WAVES
                    ).toString(),
                };
    ...
    loadRawAliases = (address) => {
        return this.getApi()
            .addresses.aliases(address)
            .then((aliasesResponse) => aliasesResponse.data);
    };
    ...
    loadAssets = async (address) => {
        ...
        const details = (
            await api.detailsMultiple(
                balanceResponse.data.balances.map(({ assetId }) => assetId)
            )
        ...
        return balanceResponse.data.balances.map((item) => {
            ...
            if (item.assetId === VostokToWavesEnterprise.id) {
                ...
                item.issueTransaction.description =
                    VostokToWavesEnterprise.description;
            }
    ...
    loadNftTokens = async (address, limit, after) => {
        const balanceResponse = await this.getApi().assets.nft(
            address,
            limit,
            after
        );
    ...
    loadScript = (address) => {
        return this.getApi()
            ...
            .then((scriptResponse) => scriptResponse.data);
    };

    loadScriptMeta = (address) => {
        return this.getApi()
            .addresses.scriptMeta(address)
            .then((metaResponse) => metaResponse.data);
    };

    validate = (address) => {
        return this.getApi()
            .addresses.validate(address)
            .then((validateResponse) => validateResponse.data.valid);
    };
    ...
    decompileScript = (scriptBase64, networkId) => {
        ...
        const api = thirdPartyApi(
            config.spamListUrl,
            this.configurationService.getDecompileScriptUrl(networkId)
        );

        return api
            .decompileScript(scriptBase64, networkId)
            .then((decompileResponse) => decompileResponse.data.script);
    };
}
