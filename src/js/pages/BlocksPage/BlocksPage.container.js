...
const derivePageBoundaries = (pageNumber, lastPage) => {
    const lowPage =
        Math.floor((pageNumber - 1) / NUMBER_OF_PAGES_IN_ROW) *
        NUMBER_OF_PAGES_IN_ROW;
...
export class BlocksPage extends React.Component {
    ...
    loadCurrentPage = (pageNumber) => {
        ...
        const from = Math.max(
            1,
            this.state.height - pageNumber * BLOCKS_PER_PAGE + 1
        );
        const to = Math.min(this.state.height, from + BLOCKS_PER_PAGE);

        ServiceFactory.forNetwork(networkId)
            .blockService()
            ...
            <div className="loaderWrapper">
                ...
                >
                    <div className="content card">
                        ...
                        <div className="headline">{pagination}</div>
                    </div>
                </Loader>
            </div>
        );
